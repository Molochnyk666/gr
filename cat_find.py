...
class category_filter:
    ...
    def load_embedings(self,name):
        if path.exists("embedings"):
            with open("embedings","rb",encoding="utf-8") as file:
    ...
    def load_embedings(self):
        ...
        for cat in self.categoryes():
                temp = list(map(lambda x: " ".join(x["content"]) if type(x["content"]) == list else x["content"],self.words(cat)))
                self.embedings[cat] = list(map(lambda x: {"embeding":x[0],"word":x[1]},zip(tuple(category_filter.model.encode(temp, convert_to_tensor=True)),self.words(cat))))
    ...
    def find_category(self,obj):
        ...
        if len(title_metrick) == 0:
            if len(desc_metrick) == 0:
                vector_tags_title = self.vectors3(norm,"TITLE")
                if vector_tags_title["score"] < 0.2:
                    return self.vectors3(norm,"DESCRIPTION")
                else:
    ...
    def vectors3(self,obj,key):
        ...
        #     print("+"*20)
        for category in self.categoryes():
            words = filter(lambda x: not category_filter.find_antiword(text,x), self.words(category))
            for item in self.embedings[category]:
                if category_filter.find_antiword(text,item["word"]):
                    continue
                cosine_scores = util.cos_sim(item["embeding"], embeddings2).item()
                res.append({"category":category,"word":item["word"]["content"],"score":cosine_scores,"type":"SBERTA"})
        try:
        ...
        for category in self.categoryes():
            ...
            if material_key:
                if not category_filter.materialDetect.has_material(obj,material_key):
                    if category != "Cardboard gift boxes":
                        continue
                else:
            ...
            # color_key = category_filter.colorDetect.detect(category)
            # if color_key:
            #     if category_filter.colorDetect.has_color(obj,color_key):
            #         flags.append({"category":category,"word":color_key,"score":2,"type":"color"})
            # material_key = category_filter.materialDetect.detect(category)
            # if material_key:
            #     if not category_filter.materialDetect.has_material(obj,material_key):
