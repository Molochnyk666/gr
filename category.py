...
class category_filter:
    ...
    def _validate_word(word):
        if isinstance(word,str):
            if str.islower(list(word.keys())[0]):
                return True
            else:
    ...
    def add_anti_word(self,category,word,anti_word):
        if category not in self.filter:
            raise Exception("No such category")
    ...
    def find_category(self,text):
        ...
        for category in self.categoryes():
            for word in self.words(category):
                if category_filter.find_antiword(flat_text,word):
                    continue
    ...
    def find_antiword(flat_text,word):
        decorated_product = category_filter.decorator_function(product)
        if "anti_word" in word:
            for prod in decorated_product(word["anti_word"],flat_text):
                if prod[0] == prod[1]:
                    return True
        return False
